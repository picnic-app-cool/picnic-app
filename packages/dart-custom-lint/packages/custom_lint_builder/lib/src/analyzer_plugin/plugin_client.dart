// DO NOT MODIFY!
// This file is a fork of analyzer_plugin/ServerPlugin.
// For the sake of being able to easily upgrade this fork when analyzer_plugin
// updates, this file should be kept as close to the original as possible.

// ignore_for_file: sort_constructors_first, prefer_foreach, prefer_final_in_for_each, prefer_final_locals, avoid_types_on_closure_parameters, comment_references, avoid_catching_errors, public_member_api_docs
...
abstract class ServerPlugin {
  ...
  Future<void> handleAffectedFiles({
    ...
    final analyzedPaths = paths.where(analysisContext.contextRoot.isAnalyzed).toList(growable: false);
  ...
  Future<AnalysisGetNavigationResult> handleAnalysisGetNavigation(AnalysisGetNavigationParams parameters) async {
    return AnalysisGetNavigationResult(<String>[], <NavigationTarget>[], <NavigationRegion>[]);
  }
  ...
  ///
  /// Throw a [RequestFailure] if the request could not be handled.
  Future<AnalysisUpdateContentResult> handleAnalysisUpdateContent(AnalysisUpdateContentParams parameters) async {
    ...
    paths.forEach((String path, Object? overlay) {
      ...
      if (overlay is AddContentOverlay) {
        ...
        if (oldContents == null) {
          ...
          throw RequestFailure(RequestErrorFactory.invalidOverlayChangeNoContent());
        }
        try {
          newContents = SourceEdit.applySequence(oldContents, overlay.edits);
        } on RangeError {
          throw RequestFailure(RequestErrorFactory.invalidOverlayChangeInvalidEdit());
        }
  ...
  Future<CompletionGetSuggestionsResult> handleCompletionGetSuggestions(
    ...
    return CompletionGetSuggestionsResult(-1, -1, const <CompletionSuggestion>[]);
  }
  ...
  ///
  /// Throw a [RequestFailure] if the request could not be handled.
  Future<EditGetRefactoringResult?> handleEditGetRefactoring(EditGetRefactoringParams parameters) async {
    return null;
  }
  ...
  ///
  /// Throw a [RequestFailure] if the request could not be handled.
  Future<PluginVersionCheckResult> handlePluginVersionCheck(PluginVersionCheckParams parameters) async {
    _sdkPath = parameters.sdkPath;
    var versionString = parameters.version;
    var serverVersion = Version.parse(versionString);
    return PluginVersionCheckResult(isCompatibleWith(serverVersion), name, version, fileGlobsToAnalyze,
        contactInfo: contactInfo);
  }

  /// Return `true` if this plugin is compatible with an analysis server that is
  /// using the given version of the plugin API.
  bool isCompatibleWith(Version serverVersion) => serverVersion <= Version.parse(version);
  ...
  /// Optionally allows handling custom [Request].
  FutureOr<ResponseResult?> handleCustomRequest(Request request, int requestTime);
  ...
  Future<Response?> _getResponse(Request request, int requestTime) async {
    ...
    result ??= await handleCustomRequest(request, requestTime);
    if (result == null) {
      return Response(request.id, requestTime, error: RequestErrorFactory.unknownRequest(request.method));
    }
  ...
  Future<void> _onRequest(Request request) async {
    ...
    try {
      ...
      response = Response(id, requestTime,
          error: RequestError(RequestErrorCode.PLUGIN_ERROR, exception.toString(), stackTrace: stackTrace.toString()));
    }
    if (response != null) {
