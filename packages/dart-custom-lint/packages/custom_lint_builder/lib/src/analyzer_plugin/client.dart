...
import 'package:analyzer_plugin/protocol/protocol_common.dart' as analyzer_plugin;
...
class Client extends ServerPlugin {
  ...
  Client(this.plugin, [analyzer.ResourceProvider? provider])
      : super(
          resourceProvider: provider ?? analyzer.PhysicalResourceProvider.INSTANCE,
        );
  ...
  Stream<CustomAnalysisNotification> _getAnalysisErrors(
    ...
    final analysisErrors = plugin
        ...
        .where(
          (lint) => !fileIgnoredCodes.contains(lint.code) && !_isIgnored(lint, lineInfo, source),
        )
        .map<analyzer_plugin.AnalysisError?>((e) => e.asAnalysisError())
        // ignore: avoid_types_on_closure_parameters
        .handleError((Object error, StackTrace stackTrace) => _handleGetLintsError(analysisResult, error, stackTrace))
        .where((e) => e != null)
  ...
  analyzer_plugin.AnalysisError? _handleGetLintsError(
    ...
    return analyzer_plugin.AnalysisError(
      ...
      analysisResult.lintLocationFromLines(startLine: 1, endLine: 2).asLocation(),
  ...
  Future<AwaitAnalysisDoneResult> _handleAwaitAnalysisDone(
    ...
    while (_pendingAnalyzeFilesFutures.isNotEmpty || _pendingGetLintsSubscriptions.isNotEmpty) {
      ...
      _hotReloader = HotReloader.create(
        onBeforeReload: (c) {
          channel.sendNotification(
            const PrintNotification('Source change detected, hot-reloading...').toNotification(),
          );
...
final _ignoreForFileRegex = RegExp(r'//\s*ignore_for_file\s*:(.+)$', multiLine: true);
...
List<ExpectLintMeta> _getAllExpectedLints(
  ...
  return expectLints.expand((expectLint) {
    ...
    return codes.map((rawCode) {
      final codeOffset = codeOffsetAcc + (rawCode.length - rawCode.trimLeft().length);
