...
class Lint {
  ...
  const Lint({
    ...
    this.correction,
    this.url,
    Stream<analyzer_plugin.AnalysisErrorFixes> Function(Lint lint)? getAnalysisErrorFixes,
  ...
  final Stream<analyzer_plugin.AnalysisErrorFixes> Function(Lint self)? _getAnalysisErrorFixes;
...
extension LintLocationFileResultExtension on FileResult {
  ...
  LintLocation lintLocationFromLines({
    ...
    final startOffset = lineInfo.getOffsetOfLine(startLine - 1) + startColumn - 1;
...
extension on Element {
  ParsedUnitResult? tryGetParsedUnit() {
    ...
    return parsedLibrary.units.firstWhereOrNull((element) => element.uri == library.source.uri);
  }
}
