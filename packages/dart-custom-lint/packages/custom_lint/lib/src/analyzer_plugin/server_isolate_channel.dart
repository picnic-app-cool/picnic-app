...
import 'package:analyzer_plugin/src/protocol/protocol_internal.dart' show RequestParams;
...
abstract class _BaseIsolateChannel {
  ...
  late final Stream<Object?> _receivePortStream = _receivePort.asBroadcastStream();

  late final _sendPort = _receivePortStream.where((event) => event is SendPort).cast<SendPort>().first;
  ...
  /// The [Notification]s emitted by the plugin
  late final Stream<PrintNotification> messages =
      notifications.where((e) => e.event == PrintNotification.key).map(PrintNotification.fromNotification);
  ...
  late final Stream<PluginErrorParams> pluginErrors = StreamGroup.mergeBroadcast([
    // Manual error notifications from the plugin
    notifications.where((e) => e.event == 'plugin.error').map(PluginErrorParams.fromNotification),

    // When the receivePort is passed to Isolate.onError, error events are
    // received as ["error", "stackTrace"]
    _receivePortStream.where((event) => event is List).cast<List>().map((event) {
  ...
  late final Stream<RequestError> responseErrors = responses.where((e) => e.error != null).map((e) => e.error!);
...
class ServerIsolateChannel extends _BaseIsolateChannel {
  ...
  late final Stream<AnalysisErrorsParams> lints =
      notifications.where((e) => e.event == 'analysis.errors').map(AnalysisErrorsParams.fromNotification);
}
...
class PluginIsolateChannel extends _BaseIsolateChannel {
  ...
  /// Lints emitted by the plugin
  late final Stream<CustomAnalysisNotification> lints =
      notifications.where((e) => e.event == 'analysis.errors').map(CustomAnalysisNotification.fromNotification);
}
