...
final _pluginNotStartedLintProvider =
    Provider.autoDispose.family<Map<String, plugin.AnalysisErrorsParams>, PluginKey>((ref, linkKey) {
  ref.cache5();
  ...
  for (final contextRoot in rootsForPlugin) {
    ...
    final allDependencies = <YamlMap>[
      if (pubSpec.nodes.containsKey('dependencies')) pubSpec.nodes['dependencies']! as YamlMap,
      if (pubSpec.nodes.containsKey('dev_dependencies')) pubSpec.nodes['dev_dependencies']! as YamlMap,
      if (pubSpec.nodes.containsKey('dependency_overrides')) pubSpec.nodes['dependency_overrides']! as YamlMap,
    ];
...
final lintsForPluginProvider =
    StreamProvider.autoDispose.family<Map<String, CustomAnalysisNotification>, PluginKey>((ref, linkKey) async* {
  ref.watch(invalidateLintsProvider);
  ref.cache5();
  if (ref.watch(includeBuiltInLintsProvider)) {
    final pluginNotStartedLint = ref.watch(_pluginNotStartedLintProvider(linkKey));

    if (pluginNotStartedLint.isNotEmpty) {
      yield* Stream.value({
        for (final entry in pluginNotStartedLint.entries) entry.key: CustomAnalysisNotification(entry.value, []),
      });
...
class _ComparableExpectLintMeta {
  ...
  bool operator ==(Object other) {
    return other is _ComparableExpectLintMeta && other.code == code && other.line == line;
  }
}
...
plugin.AnalysisErrorsParams _applyExpectLint(
  ...
  final unfulfilledExpectedLints = lintsForFile.whereNotNull().expand((e) => e.expectLints).toList();

  final lintsExcludingExpectedLints = lintsForFile.whereNotNull().expand((e) => e.lints.errors).where((lint) {
    ...
    if (allExpectedLints.contains(matchingExpectLintMeta)) {
      ...
      unfulfilledExpectedLints.removeWhere(
        (e) => e.line == matchingExpectLintMeta.line && e.code == matchingExpectLintMeta.code,
      );
      return false;
    }
    return true;
  ...
  return plugin.AnalysisErrorsParams(filePath, [
    ...
    for (final unfulfilledExpectedLint in unfulfilledExpectedLints)
      plugin.AnalysisError(
        ...
        correction: 'Either update the code such that it emits the lint ${unfulfilledExpectedLint.code} '
...
final allLintsProvider = Provider.autoDispose<Map<String, plugin.AnalysisErrorsParams>>((ref) {
  ref.cache5();
      ...
      (previous, next) {
        ...
        for (final fileToUpdate in allFiles) {
          ...
          final unfilteredLintsForFile = linkKeys
              .map(
                (link) => ref.read(lintsForPluginProvider(link)).asData?.value[fileToUpdate],
              )
              .toList();
