...
final _pluginSourceChangeProvider = StreamProvider.autoDispose.family<void, PluginKey>((ref, pluginRootUri) {
...
final _pluginLinkProvider = FutureProvider.autoDispose.family<PluginLink, PluginKey>((ref, pluginRootUri) async {
  ref.cache5();
  ref.watch(_pluginSourceChangeProvider(pluginRootUri));

  final pluginName = ref.watch(pluginMetaProvider(pluginRootUri).select((value) => value.name));
...
final versionCheckProvider = StateProvider<plugin.PluginVersionCheckParams?>((ref) => null);

final _versionInitializedProvider = FutureProvider.autoDispose.family<void, PluginKey>((ref, linkKey) async {
  ref.cache5();
...
final pluginMetaProvider = Provider.autoDispose.family<Package, PluginKey>((ref, linkKey) {
  ref.cache5();
...
final pluginMetasForContextRootProvider =
    Provider.autoDispose.family<List<Package>, plugin.ContextRoot>((ref, contextRoot) {
  ref.cache5();
  Iterable<Package> _getPluginsForContext(
    ...
    for (final dependency
        in {...pubspec.dependencies, ...pubspec.devDependencies, ...pubspec.dependencyOverrides}.entries) {
      ...
      final dependencyPubspec = _loadPubspecAt(dependencyMeta.root.toFilePath());
...
final contextRootsForPluginProvider = Provider.autoDispose.family<List<plugin.ContextRoot>, PluginKey>(
  (ref, packageUri) {
    ref.cache5();
...
final _contextRootInitializedProvider = FutureProvider.autoDispose.family<void, PluginKey>((ref, linkKey) async {
  ref.cache5();
  final link = await ref.watch(_pluginLinkProvider(linkKey).future);
...
final priorityFilesProvider = StateProvider<plugin.AnalysisSetPriorityFilesParams?>((ref) => null);

final _priorityFilesInitializedProvider = FutureProvider.autoDispose.family<void, PluginKey>((ref, linkKey) async {
  ref.cache5();
  final link = await ref.watch(_pluginLinkProvider(linkKey).future);
...
final _configInitializedProvider = FutureProvider.autoDispose.family<void, PluginKey>((ref, linkKey) async {
  ref.cache5();
  final link = await ref.watch(_pluginLinkProvider(linkKey).future);
...
final pluginLinkProvider = FutureProvider.autoDispose.family<PluginLink, PluginKey>((ref, linkKey) async {
  ref.cache5();
  final link = await ref.watch(_pluginLinkProvider(linkKey).future);
...
final allPluginLinksProvider = FutureProvider.autoDispose<Map<PluginKey, Result<PluginLink>>>((ref) async {
  ref.cache5();
...
extension on Pubspec {
  bool hasDependency(String name) {
    return dependencies.containsKey(name) || devDependencies.containsKey(name) || dependencyOverrides.containsKey(name);
  }
}
