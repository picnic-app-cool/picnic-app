...
class Result<T> {
  ...
  Result.data(T value) : map = <R>({required data, required error}) => data(value);

  /// A [Result] in error state
  Result.error(Object err, StackTrace stack) : map = <R>({required data, required error}) => error(err, stack);
  ...
  bool operator ==(Object? other) {
    return other is Result<T> && other.error == error && other.stackTrace == stackTrace && other.value == value;
  }
