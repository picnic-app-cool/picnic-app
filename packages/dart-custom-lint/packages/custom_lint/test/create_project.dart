...
'''
    ...
    packageConfig: omitPackageConfig || pubpsec != _pluginDefaultPubspec ? null : createPackageConfig(name: name),
    name: name,
  );
}
...
String createPackageConfig({
  ...
  return jsonEncode({
    ...
    'packages': <Object?>[
      ...(PeerProjectMeta.current.exampleLintPackageConfig['packages']! as List<Object?>).cast<Map>().where(
            (e) =>
                e['name'] != 'custom_lint' &&
      ...
      for (final plugin in plugins.entries)
        {'name': plugin.key, 'rootUri': plugin.value.toFilePath(), 'packageUri': 'lib/', 'languageVersion': '2.17'},
      <String, String>{'name': name, 'rootUri': '../', 'packageUri': 'lib/', 'languageVersion': '2.17'},
      <String, String>{
        'name': 'custom_lint',
  ...
  return createTmpFolder(
    {
      ...
      if (packageConfig != null) '.dart_tool/package_config.json': packageConfig,
    },
    name,
  );
}
